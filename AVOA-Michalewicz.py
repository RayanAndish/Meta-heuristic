...
with open('AVOA-Michalewicz-D30-25000.csv', mode='w') as results_file:
    results_writer = csv.writer(results_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
    results_writer.writerow(['Iteration', 'Best_Solutions', 'Best_Fitness', 'Execution_Time'])

    lb_micha = [0] * dim
    ub_micha = [np.pi] * dim
    all_solutions, all_fitness, execution_time = AVA(michalewicz, lb=lb_micha, ub=ub_micha, n_vultures=10,
                                                     n_iterations=25000)
