...
def initialize_population(bees, dim, search_space):
    return np.random.uniform(search_space[0][0], search_space[0][1], size=(bees, dim))
...
def employeed_bee_phase(employed_bees, search_space):
    ...
    new_solutions = np.clip(new_solutions, search_space[0][0], search_space[0][1])
...
with open('HBM-Branin-D4-25000.csv', mode='w') as results_file:
    ...
    Bbounds = [(-5, 10), (0, 15), (-5, 10), (0, 15)]
    result = hbm_algorithm(func=branin, bees=50, dim=4, search_space=Bbounds, iterations=25000)
