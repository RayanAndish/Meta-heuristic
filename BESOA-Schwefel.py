...
with open('BESOA-Schwefel-D30-25000.csv', mode='w') as results_file:
    ...
    for t in range(max_iter):
        ...
        lb_sch = np.array([-500])
        ub_sch = np.array([500])
        Xbest, Gbest, CNVG = BESOA(func=schwefel, lb=lb_sch, ub=ub_sch, dim=dim, N=N, max_iter=t+1, pa=pa, pb=pb, pc=pc, ps=ps, pm=pm, pe=pe)
        end_time = time.time()
