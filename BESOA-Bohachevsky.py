...
with open('BESOA-Bohachevsky-D4-25000.csv', mode='w') as results_file:
    ...
    for t in range(max_iter):
        ...
        lb_boha = (-100) * 2
        ub_boha = (100) * 2
        Xbest, Gbest, CNVG = BESOA(func=bohachevsky, lb=lb_boha, ub=ub_boha, dim=dim, N=N, max_iter=t+1, pa=pa, pb=pb, pc=pc, ps=ps, pm=pm, pe=pe)
        end_time = time.time()
