...
with open('TSOA-Ackley-D30-25000.csv', mode='w') as results_file:
    ...
    best_fitness, best_tuna, all_solutions = tso(ackley, dim=dim, lb=lb_ack, ub=ub_ack, max_iter=max_iter, num_tuna=num_tuna, w=w, c1=c1, c2=c2)
    end_time = time.time()

    # Print Value of Each Iteration
    for t, solution in enumerate(all_solutions):
        best_fitness_t = np.min([ackley(p) for p in solution])
        best_solution_t = solution[np.argmin([ackley(p) for p in solution]), :]
