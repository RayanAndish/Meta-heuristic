...
with open('COA-Styblinski-D30-25000.csv', mode='w') as results_file:
    ...
    lb_sty = [-5] * dim
    ub_sty = [5] * dim
    best_fitness, best_solution, all_solutions = chitah_optimizer(styblinski_tang, dim=dim, pop=population_size, iter=max_iterations, lb=lb_sty, ub=ub_sty)
    end_time = time.time()

    for t in range(max_iterations):
        best_fitness_t = evaluate_fitness(all_solutions[t], styblinski_tang).min()
        best_solution_t = all_solutions[t][np.argmin(evaluate_fitness(all_solutions[t], styblinski_tang)), :]
