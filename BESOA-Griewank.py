...
with open('BESOA-Griewank-D30-25000.csv', mode='w') as results_file:
    ...
    for t in range(max_iter):
        ...
        lb_gri = (-600)
        ub_gri = (600)
        Xbest, Gbest, CNVG = BESOA(func=griewank, lb=lb_gri, ub=ub_gri, dim=dim, N=N, max_iter=t + 1, pa=pa, pb=pb, pc=pc, ps=ps, pm=pm, pe=pe)
        end_time = time.time()
