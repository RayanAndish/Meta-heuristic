...
with open('COA-Ackley-D10-30.csv', mode='w') as results_file:
    ...
    lb_ack = [-32] * dim
    ub_ack = [32] * dim
    best_fitness, best_solution, all_solutions = chitah_optimizer(ackley, dim=dim, pop=population_size, iter=max_iterations, lb=lb_ack, ub=ub_ack)
    end_time = time.time()

    for t in range(max_iterations):
        best_fitness_t = evaluate_fitness(all_solutions[t], ackley).min()
        best_solution_t = all_solutions[t][np.argmin(evaluate_fitness(all_solutions[t], ackley)), :]
