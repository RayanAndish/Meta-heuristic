...
with open('AFHA-Branin-D4-25000.csv', mode='w') as results_file:
    results_writer = csv.writer(results_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
    results_writer.writerow(['Iteration', 'Best_Solutions', 'Best_Fitness', 'Execution_Time'])

    dim = 4
    Bbounds = [(-5, 10), (0, 15)] * (dim // 2)
    best_fish, best_fitness, all_solutions, all_fitness, execution_time = archer_fish_hunter(func=branin, n=50, dim=4, max_iter=25000, bounds=Bbounds)

    for t in range(1, len(all_solutions)):  # Start from 1 to avoid duplicating the header
        ...
        print(f"Iteration {t}: Best Fitness = {best_fitness_t}")
        print(f"Best Solutions: {best_solution_t}")

        results_writer.writerow([t, best_solution_t.tolist(), best_fitness_t, execution_time])