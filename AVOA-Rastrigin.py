...
def AVA(obj_func, lb, ub, n_vultures, n_iterations):
    ...
    for t in range(n_iterations):
        ...
        #Add information to Each Itteration
        all_solutions.append(vultures.copy())
        all_fitness.append(fitness.copy())
...
# Define the dimension for the Rastrigin function
dim = 30

# Run the algorithm with functions and store the results in a CSV file
with open('AVOA-Rastrigin-D30-25000.csv', mode='w') as results_file:
    results_writer = csv.writer(results_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
    results_writer.writerow(['Iteration', 'Best_Solutions', 'Best_Fitness', 'Execution_Time'])

    lb_rast = [-5] * dim
    ub_rast = [12] * dim
    all_solutions, all_fitness, execution_time = AVA(rastrigin, lb=lb_rast, ub=ub_rast, n_vultures=10,
                                                     n_iterations=25000)
