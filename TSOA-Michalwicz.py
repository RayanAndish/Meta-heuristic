...
with open('TSOA-Michalewicz-D30-25000.csv', mode='w') as results_file:
    ...
    best_fitness, best_tuna, all_solutions = tso(michalewicz, dim=dim, lb=lb_micha, ub=ub_micha, max_iter=max_iter, num_tuna=num_tuna, w=w, c1=c1, c2=c2)
    end_time = time.time()

    # Print Value of Each Iteration
    for t, solution in enumerate(all_solutions):
        best_fitness_t = np.min([michalewicz(p) for p in solution])
        best_solution_t = solution[np.argmin([michalewicz(p) for p in solution]), :]
