...
with open('BESOA-Ackley-D10-10000.csv', mode='w') as results_file:
    ...
    for t in range(max_iter):
        ...
        lb_ack = (-32)
        ub_ack = (32)
        Xbest, Gbest, CNVG = BESOA(func=ackley, lb=lb_ack, ub=ub_ack, dim=dim, N=N, max_iter=t+1, pa=pa, pb=pb, pc=pc, ps=ps, pm=pm, pe=pe)
        end_time = time.time()
