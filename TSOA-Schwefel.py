...
with open('TSOA-Schwefel-D30-25000.csv', mode='w') as results_file:
    ...
    best_fitness, best_tuna, all_solutions = tso(schwefel, dim=dim, lb=lb_sch, ub=ub_sch, max_iter=max_iter, num_tuna=num_tuna, w=w, c1=c1, c2=c2)
    end_time = time.time()

    # Print Value of Each Iteration
    for t, solution in enumerate(all_solutions):
        best_fitness_t = np.min([schwefel(p) for p in solution])
        best_solution_t = solution[np.argmin([schwefel(p) for p in solution]), :]
