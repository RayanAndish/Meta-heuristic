...
with open('BESOA-Michalwicz-D30-25000.csv', mode='w') as results_file:
    ...
    for t in range(max_iter):
        ...
        lb_mich = (0)
        ub_mich = (np.pi)
        Xbest, Gbest, CNVG = BESOA(func=michalewicz, lb=lb_mich, ub=ub_mich, dim=dim, N=N, max_iter=t+1, pa=pa, pb=pb, pc=pc, ps=ps, pm=pm, pe=pe)
        end_time = time.time()
