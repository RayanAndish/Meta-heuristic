...
with open('COA-Rosenbrock-D30-25000.csv', mode='w') as results_file:
    ...
    lb_rosen = [-5] * dim
    ub_rosen = [10] * dim
    best_fitness, best_solution, all_solutions = chitah_optimizer(rosenbrock, dim=dim, pop=population_size, iter=max_iterations, lb=lb_rosen, ub=ub_rosen)
    end_time = time.time()

    for t in range(max_iterations):
        best_fitness_t = evaluate_fitness(all_solutions[t], rosenbrock).min()
        best_solution_t = all_solutions[t][np.argmin(evaluate_fitness(all_solutions[t], rosenbrock)), :]
