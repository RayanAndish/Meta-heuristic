    ...
    for column in df.columns:
        if column != 'Iteration':
            if df[column].apply(type).eq(list).any():
                ...
                result_df[column + '_Variance'] = [
                    sum((x - (sum(column_values) / len(column_values))) ** 2 for x in column_values) / len(
                        column_values)]
                result_df[column + '_Standard_Deviation'] = [(sum(
                    (x - (sum(column_values) / len(column_values))) ** 2 for x in column_values) / len(
                    column_values)) ** 0.5]
...
if __name__ == '__main__':
    # تعداد پردازنده‌ها می‌تواند برابر تعداد هسته‌های پردازنده شما باشد
    num_processors = os.cpu_count()

    # تعداد پردازنده‌ها را محدود کنید
    limited_processors = int(num_processors * 0.5)

    with Pool(limited_processors) as pool:
